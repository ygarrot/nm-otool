...
/*   Updated: 2019/01/24 16:39:15 by ygarrot          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	set_section_values(void *ptr, t_otool *otool)
{
	...
	if (otool->head.magic == MH_MAGIC_64 || otool->head.magic == MH_CIGAM_64)
		otool->section = (t_sec){
		.size = get_int_indian(otool, section_64->size),
		.ptr = otool->head.ptr + get_int_indian(otool, section_64->offset),
		.addr = section_64->addr};
	else
	{
		otool->section = (t_sec){
		.size = get_int_indian(otool, section->size),
		.ptr = otool->head.ptr + get_int_indian(otool, section->offset),
		.addr = section->addr};
	}
}
...
void	cross_command(void *ptr, void *struc, uint32_t index)
{
	...
	if (!ptr || !struc)
		return ;
	lc = ptr;
	cmd = get_int_indian(struc, lc->cmd);
